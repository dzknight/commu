...
<!--
	회원가입 페이지
	1. 회원가입을 위한 폼을 작성합니다.
	2. 회원가입을 위한 폼은 html로 작성합니다.
	3. 회원가입을 위한 폼은 post방식으로 전송합니다.
	4. 회원가입을 위한 폼은 enctype="multipart/form-data"로 설정합니다.
	5. 회원가입을 위한 폼은 action="/memberjoind"로 설정합니다.
	6. 회원가입을 위한 폼은 method="post"로 설정합니다.
-->
   ...
   select, button {
    ...
    .passport-info {
       ...
       .error {
        ...
        .form-group #photoValidation {
            margin-top: 10px;
        }
        .form-group #photoValidation span {
            display: block;
            margin-top: 5px;
        }
        .form-group #photoValidation .error {
            color: red;
        }
...
function searchZipcode() {
	...
	<h3>회원가입</h3>
        ...
        <ul>
            ...
            <li>권장 크기: 413 x 531 픽셀</li>
            <li>허용 범위: 395~431 x 507~550 픽셀</li>
            <li>해상도: 300dpi 권장</li>
            <li>6개월 이내 촬영된 사진</li>
        </ul>
    </div>
	...
	<form action="${pageContext.request.contextPath}/memberjoind" id="myForm" method="post" enctype="multipart/form-data">
		...
		<div>
			...
			<button type="button" id="duplexid"  > 유효성체크</button>

			<label for="password">비밀번호:</label>
            <input type="password" name="password" id="password" pattern=".{8,}" title="8자 이상의 비밀번호를 입력하세요" required />

            <label for="confirm-password">비밀번호 확인:</label>
            <input type="password" name="confirmPassword" id="confirmPassword" placeholder="비밀번호를 다시 입력하세요"  required/>
		    <small class="validation-message" id="confirmMessage"></small>
        ...
        <div class="form-group">
            ...
            <input type="file" id="profileImage" name="profileImageFile" accept="image/jpeg,image/jpg">
            <div id="photoValidation"></div>
            <img id="photoPreview" class="profile-preview" style="display: none;">
        </div>
...
$("#duplexid").click(()=>{  // id가 userid인 태그를 선택해서 click이벤트를 정의한다
    ...
    <!-- 휴대폰 번호 숫자만 들어가게  검사 -->
        $('input[name="mobile1"], input[name="mobile2"], input[name="mobile3"]').on('input', function() {
